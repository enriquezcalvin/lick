...
export default {
  ...
  computed: {
    tableData () {
      return this.$store.getters['OtherMaterials/getOtherMaterials']
    }
  },
  watch: {
  ...
  methods: {
    add_other_materials: function () {
      this.loading = true
      this.$store.dispatch('OtherMaterials/createOtherMaterial', this.material)
        .then((data) => {
          this.opened = false
          this.loading = false
          this.$q.notify({
